...
**Allocate the design budget here too.** The problems are rarely peers — one is often the mechanism behind another — so ask which get design work and which is load-bearing, before any research runs. The user already holds that answer, and stage 5 is far too late to learn it. Weigh it against the team's current goal metrics, fetched at this gate and never before the walk ([references/alignment-doc.md](references/alignment-doc.md)).
...
- **One research-and-ideation agent per problem** — the fan-out unit; each agent owns a single problem end to end.
  - **Research the precedent.** How well-regarded products solve that problem, with **at least one primary source** (the product's own docs, changelog, release notes, or a first-hand teardown), naming the specific surface examined. Aggregator listicles find candidates but are never a citation, and **no statistic without a primary source** — a vendor's conversion claim is marketing until you can point at where it's measured. Two or three worked examples beat a survey.
  - **Then ideate against it.** **Three divergent ideas argued in prose**, each naming the problem and the critique point it answers, plus **the signal that would show it worked** — a direction with no observable outcome is a proposal, not something a team can ship and check. Three is the *generation* target, not the output contract; the gate below decides what survives. Route them through `impeccable` in `shape` mode to sharpen.
- **Converge before anything is drawn.** With every problem's ideas sharpened, read them all against each other in one pass and take that read to the user: which theses collapse into one, what composes and what's mutually exclusive, and a recommended bundle in ship order — ship now / this quarter / rewrite — from the directions' own cost notes. One field is required rather than lucky: **the cheapest change that repairs the walked blocker, even when it is none of the directions.** Keep sharpening *before* this gate — `impeccable`'s corrections are often what decide whether a direction is viable, which is exactly what the gate needs to choose on. Then ask what gets wireframed; the count is an output of that answer, so one problem may carry four frames and another one. Draw to a fixed quota instead and the run discovers its own duplicates only after paying to research, argue and draw them.
- **Wireframes illustrate ideas; they never produce them.** Draw only **what the gate selected**, and only once the ideas exist as prose, grounded in the surface's current code. The test: **if an idea can't be written as a paragraph before it's drawn, it isn't an idea yet.** Sketch first and you end up reading the rationale off your own drawings, with justifications retrofitted onto them. Fidelity and framing: [references/wireframe-style.md](references/wireframe-style.md), read before drawing. Render mechanics and the fallback: [references/open-design.md](references/open-design.md).
...
Report what you handed off and what came back, and end on the gate's recommendation — an unranked menu of alternatives is a stall for a team working in weeks. Don't editorialise a second critique on top; recommending a bundle isn't re-arguing `impeccable`'s verdict.
...
- **Opus** — scoring and clustering (stage 3), each problem's idea synthesis, the convergence read, and `impeccable critique`. Deciding what a cluster of findings *means*, what collapses into what, and what to build instead is the part that degrades when you economise.
