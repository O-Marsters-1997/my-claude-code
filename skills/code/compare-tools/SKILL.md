...
"something for caching" — invoke the `grilling` skill to resolve the decision tree:
scale, read/write mix, latency, hosting constraints, team familiarity, existing
infrastructure, etc. Skip `grilling` if the use case is already crisp.
...
1. **Context** — one paragraph: codebase characterisation + the confirmed use case. If `grilling`
...
- [ ] Use case was confirmed or clarified (via `grilling` if fuzzy)
